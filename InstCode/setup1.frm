...
'
    ...
    'paultest - Key check
    If Not DEMO Then
       Dim UserSetupKey, Response
       Dim vSetupOK, vContinue As Integer
       vSetupOK = False
       vContinue = True
       While vContinue = True
          UserSetupKey = InputBox("Enter Setup Key located on back of CD-ROM cover", Me.Caption, "")
          If Trim(UserSetupKey) <> "" Then
             UserSetupKey = Trim(UserSetupKey)
             If UserSetupKey = gSetupKey Then
                vSetupOK = True
                vContinue = False
             Else
                Response = MsgBox("Incorrect setup key. Retry?", vbYesNo, Me.Caption)
                If Response = vbNo Then vContinue = False
             End If
          Else
             Response = MsgBox("Incorrect setup key. Retry?", vbYesNo, Me.Caption)
             If Response = vbNo Then vContinue = False
          End If
       Wend
       If Not vSetupOK Then ExitSetup Me, gintRET_FATAL
    End If
    'End Key check
...
'Write ini file 'paultest
If Not IsWorkstation Then gDatabasePath = gstrDestDir & "bnb1.mdb"
WriteIniSettings
    '
...
Public Sub WriteIniSettings()
    ...
    iniFile = winDir & gstrAppName & ".INI"
'MsgBox gDatabasePath 'paultest
    X = WritePrivateProfileString("Connect", "DatabaseName", gDatabasePath, iniFile)
    X = WritePrivateProfileString("Connect", "SetupDirectory", gstrDestDir, iniFile)
