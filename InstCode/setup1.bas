...
'
Public gDatabasePath As String  'global database path paultest
Public IsWorkstation As Integer
Public Const gSetupKey$ = "A1000" 'paultest - key
    ...
    Do While ReadSetupFileLine(strSection, intIdx, sFile) = True
        ...
        If sFile.strDestDir <> gstrNULL Then
            ...
            If UCase(strDestName) = gstrFILE_WINT351 Then
                ...
                End If
            End If
            'Special case for the database file and report files.
            'Don't write out file if installing as a workstation.
            If IsWorkstation And (UCase(strDestName) = mstrFILE_BNB1 Or UCase(strDestName) Like "*.RPT") Then
               intRC = IDIGNORE
               'LogNote ResolveResString(resCOMMON_RICHED32NOTCOPIED, "|1", strDestName)
               AbortAction
            End If
            ...
            'VerInstallFile does its thing which involves a full file read (for
            ...
            'paultest - Use version info from the file itself, not from
            'the setup.lst file.
