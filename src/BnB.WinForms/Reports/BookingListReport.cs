    ...
    private void ComposeContent(IContainer container)
    {
        container.Column(column =>
        {
            ...
            column.Item().PaddingBottom(10).Row(row =>
            {
                ...
                row.RelativeItem().AlignRight().Text($"Total Nights: {_bookings.Sum(b => b.Nights ?? 0)} | Total Guests: {_bookings.Sum(b => b.NumberOfGuests ?? 1)}").FontSize(10);
            });
            ...
            column.Item().Table(table =>
            {
                ...
                foreach (var booking in _bookings)
                {
                    ...
                    table.Cell().TableCell(alternate).AlignCenter().Text((booking.Nights ?? 0).ToString()).TableCellText();
                    table.Cell().TableCell(alternate).AlignCenter().Text((booking.NumberOfGuests ?? 1).ToString()).TableCellText();
            ...
            column.Item().PaddingTop(5).Table(table =>
            {
                ...
                foreach (var propertyGroup in byProperty)
                {
                    ...
                    table.Cell().TableCell(alternate).AlignCenter().Text(propertyGroup.Sum(b => b.Nights ?? 0).ToString()).TableCellText();
                    table.Cell().TableCell(alternate).AlignCenter().Text(propertyGroup.Sum(b => b.NumberOfGuests ?? 1).ToString()).TableCellText();
